...
--grab what we need from flower instead of hannape
...
function M:setLeft(left)
    local playFunc = function(self, context)
        for i, v in ipairs(self._targets) do
        	local x,y = DisplayObject.getPos(v)
            DisplayObject.setPos(v, left,y)
        end
    end
...
function M:setRight(right)
    local playFunc = function(self, context)
        for i, v in ipairs(self._targets) do
        	local x,y = DisplayObject.getPos(v)
            DisplayObject.setPos(v, x+DisplayObject.getWidth(v),y)
        end
    end
    local command = self:newCommand(playFunc)
    self:addCommand(command)
...
function M:setBottom(bottom)
    local playFunc = function(self, context)
        for i, v in ipairs(self._targets) do
        	local x,y = DisplayObject.getPos(v)
            DisplayObject.setPos(v, x, bottom- DisplayObject.getHeight(v))
        end
    end
    local command = self:newCommand(playFunc)
    self:addCommand(command)
